...
function passwordStrength(pw: string): { label: string; color: string } {
  ...
  if (!hasMix) return { label: "Weak", color: "text-amber-500" };
...
function SignupForm() {
  ...
  useEffect(() => {
    ...
    if (interval) sessionStorage.setItem("signup_interval", interval);
  }, [searchParams]);
    ...
    >
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm" style={{ color: "var(--color-text-muted)" }}>Create your account</p>
        </div>
        ...
        >
          ...
          <form onSubmit={(e) => void handleSubmit(e)} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-1.5">
              <label htmlFor="name" className="text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>Full name</label>
              <input
                id="name" type="text" autoComplete="name" required minLength={2}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <button type="button" onClick={() => setShowPassword((v) => !v)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Toggle password">
            ...
            {/* Confirm password */}
            <div className="flex flex-col gap-1.5">
              <label htmlFor="confirm" className="text-xs font-medium" style={{ color: "var(--color-text-secondary)" }}>Confirm password</label>
              <div className="relative">
                <input
                ...
                <button type="button" onClick={() => setShowConfirm((v) => !v)}
                  className="absolute right-3 top-1/2 -translate-y-1/2 transition-colors" style={{ color: "var(--color-text-muted)" }} aria-label="Toggle confirm password">
                  <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                    {showConfirm
                      ? <path strokeLinecap="round" strokeLinejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.88 9.88l-3.29-3.29m7.532 7.532l3.29 3.29M3 3l3.59 3.59m0 0A9.953 9.953 0 0112 5c4.478 0 8.268 2.943 9.543 7a10.025 10.025 0 01-4.132 5.411m0 0L21 21" />
                      : <><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></>}
